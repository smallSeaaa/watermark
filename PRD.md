...
- **模板管理**：支持保存、加载和删除水印设置模板，便于重复使用常用配置
- **设置记忆**：程序启动时自动加载上次关闭时的所有设置，提高用户体验

### 2.2 自定义选项

- **字体选择**：支持选择系统安装的字体，自动获取并显示可用字体列表
- **字体大小**：允许用户自定义水印文本的字体大小
- **字体样式**：支持粗体和斜体文本样式设置
- **文本颜色**：允许用户自定义水印文本的颜色，支持两种颜色选择方式：
  - 预定义颜色列表快速选择
  - 颜色调色板对话框精确选择
- **水印透明度**：允许用户调整水印文本的透明度（0表示完全不透明，100表示完全透明）
- **水印位置**：支持多种水印位置选项，包括：
  - 四角位置：top-left、top-right、bottom-left、bottom-right
  - 边缘中心：top、bottom、left、right
  - 图片中心：center
- **样式效果**：增强水印在复杂背景下的可读性：
  - 阴影效果：可选择启用/禁用，自定义阴影颜色和偏移量
  - 描边效果：可选择启用/禁用，自定义描边颜色和宽度
...
- 格式验证和边界检查
- 默认值处理
- 透明度处理：值越大，水印越透明（0表示完全不透明，100表示完全透明）
...
- 水印样式效果设置（阴影和描边）
- 透明PNG支持和处理
- **实时预览功能**：所有参数调整可在主预览窗口中即时显示，支持图片列表切换查看不同图片效果
- **位置控制功能**：
  - 预设九宫格布局（四角 + 中心），一键快速放置水印
  - 支持鼠标拖拽在预览图上自由移动水印，并实时保存位置信息
  - 支持相对坐标计算，确保在不同尺寸图片上保持相同的相对位置
- **模板管理功能**：
  - 保存当前水印设置为模板
  - 加载已保存的模板
  - 删除模板
  - 模板保存在templates文件夹下的watermark_templates.json文件中
- **设置记忆功能**：
  - 程序退出时自动保存所有当前设置
  - 程序启动时自动加载上次退出时的设置
  - 设置保存在settings.json文件中
